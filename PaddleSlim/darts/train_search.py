...
def main(args):
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(data_prog, startup_prog):
            ...
            learning_rate = fluid.layers.cosine_decay(
                args.learning_rate, 8 * step_per_epoch, args.epochs)
    ...
    arch_progs_list[1] = fluid.CompiledProgram(arch_progs_list[
        1]).with_data_parallel(
            ...
            build_strategy=build_strategy,
            exec_strategy=exec_strategy)
    arch_progs_list[2] = fluid.CompiledProgram(arch_progs_list[
        2]).with_data_parallel(
            loss_name=fetch[2].name,
            build_strategy=build_strategy,
            exec_strategy=exec_strategy)
    arch_progs_list[3] = fluid.CompiledProgram(arch_progs_list[
        3]).with_data_parallel(
            loss_name=fetch[3].name,
            build_strategy=build_strategy,
            exec_strategy=exec_strategy)
    arch_progs_list[4] = fluid.CompiledProgram(arch_progs_list[
        4]).with_data_parallel(
            loss_name=fetch[4].name,
            build_strategy=build_strategy,
            exec_strategy=exec_strategy)
    arch_progs_list[5] = fluid.CompiledProgram(arch_progs_list[
        5]).with_data_parallel(
