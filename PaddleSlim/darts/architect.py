...
def compute_unrolled_step(image_train, label_train, image_val, label_val,
    ...
    with fluid.program_guard(arch_optim_prog, startup_prog):
        ...
        for i, (var, grad) in enumerate(leader_grads):
            #    leader_grads[i] = (var, grad - fluid.layers.elementwise_mul(fluid.layers.elementwise_div(fluid.layers.elementwise_sub(arch_grads_pos[i], arch_grads_neg[i]), fluid.layers.elementwise_mul(eps, fluid.layers.assign(np.array([2], dtype=np.float32)))), lr))
